...
def start_of_code(pyc_list, cur = 0):
    ...
    while pyc_list[cur] != TYPE_CODE and pyc_list[cur+17] != TYPE_STRING:
        cur += 1
...
def end_of_code(pyc_list, cur = 0):
    ...
    for dummy in range(n_const):
        if pyc_list[cur] == TYPE_INTEGER:
            cur += 5
        elif pyc_list[cur] == TYPE_NONE:
            cur += 1
        elif pyc_list[cur] == TYPE_CODE:
...
def is_func_def(cur, pycbuf):
    ...
    if pycbuf[cur] == LOAD_CONSTANT and pycbuf[cur+3] == MAKE_FUNCTION:
        return True;
    else:
        return False;
